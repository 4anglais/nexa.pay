...
function PayrollPage() {
  ...
  const [settings, setSettings] = useState<CompanySettings>({ paye_rate: 0, napsa_rate: 5, nhima_rate: 1 });
  ...
  useEffect(() => {
    Promise.all([
      supabase.from("employees").select("id, full_name, basic_salary, allowances"),
      supabase.from("deductions").select("*"),
      supabase.from("allowances").select("*"),
      supabase.from("company_settings").select("*").limit(1).single(),
    ]).then(([empRes, dedRes, allRes, setRes]) => {
      setEmployees((empRes.data as Employee[]) ?? []);
      setDeductions((dedRes.data as Deduction[]) ?? []);
      setAllowanceRows((allRes.data as Allowance[]) ?? []);
  ...
  const getEmployeeGross = (emp: Employee) => {
    ...
    const totalAllowances = empAllowances.length > 0
  ...
  const runPayroll = async () => {
    ...
    try {
      const { data: run, error: runErr } = await supabase
        .from("payroll_runs")
        .insert({ month: months[parseInt(month) - 1], year })
        .select()
        .single();
      ...
      setResult(`Payroll run completed! ${payslips.length} payslips generated for ${months[parseInt(month) - 1]} ${year}.`);
    } catch (err) {
  ...
  return (
    <>
      <PageHeader title="Run Payroll" description="Generate payslips for your employees" />

      <div className="mx-auto max-w-4xl space-y-6">
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-card-foreground">Select Period</h2>
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-end">
              <Button onClick={runPayroll} disabled={running || employees.length === 0}>
                {running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" strokeWidth={1.5} />}
                Run Payroll
              </Button>
            </div>
          </div>
          {result && (
            <div className={`mt-4 rounded-lg px-4 py-3 text-sm font-semibold ${result.startsWith("Error") ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success"}`}>
              {result}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-card-foreground">Payroll Preview ({employees.length} employees)</h2>
                ...
                {employees.map((emp) => {
                  ...
                  return (
                    <tr key={emp.id} className="border-b border-border last:border-0">
                      <td className="px-4 py-3 font-semibold text-card-foreground">{emp.full_name}</td>
                      <td className="px-4 py-3 text-right text-card-foreground">{formatCurrency(gross)}</td>
                      <td className="px-4 py-3 text-right text-destructive">{formatCurrency(totalDed)}</td>
                      <td className="px-4 py-3 text-right font-bold text-success">{formatCurrency(gross - totalDed)}</td>
                    </tr>
                  );
                })}
                {employees.length === 0 && (
                  <tr><td colSpan={4} className="px-4 py-12 text-center font-light italic text-muted-foreground">No employees found. Add employees first.</td></tr>
                )}
              </tbody>
            </table>
          </div>
